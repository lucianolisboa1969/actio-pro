...
export default function Home() {
  ...
  // Dias historicamente completos (todos os exercÃ­cios com pelo menos 1 log)
  ...
  // Dia atual: se treinou hoje â mostra esse dia; senÃ£o â primeiro dia incompleto
    ...
    return (
      <Layout title="Meus Treinos">
        <div className="pt-6 text-center">
          ...
          <p className="text-muted text-sm">Seu plano de treino aparecerÃÂ¡ aqui.</p>
        </div>
      </Layout>
    )
  }
  ...
  return (
    <Layout title="Meus Treinos">
      <div className="pt-4 space-y-4">
        {/* Sub-header */}
        <p className="text-muted text-sm">Gerencie seus planos e exercÃÂ­cios</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-card-red border border-red-900/30 rounded-xl p-4">
            ...
            <p className="text-muted text-xs mt-0.5">de {todayExercises.length} exercÃÂ­cios</p>
          </div>
          ...
          <div className="bg-card-green border border-green-900/30 rounded-xl p-4">
            ...
            <p className="text-muted text-xs mt-0.5">de {totalExercises} exercÃÂ­cios</p>
          </div>

          <div className="bg-card-teal border border-teal-900/30 rounded-xl p-4">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs text-muted uppercase tracking-wide">Total ExercÃÂ­cios</span>
          ...
          allTodayDone ? (
            <div className="card border-primary/20 bg-primary/5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <p className="text-primary font-semibold text-sm">Dia {currentDay} completo! Ã°ÂÂÂ</p>
              </div>
              {nextDayExercises.length > 0 && (
                <div className="mt-1 pt-2 border-t border-border/40">
                  <p className="text-muted text-xs">PrÃÂ³ximo Ã¢ÂÂ Dia {nextDay}</p>
                  <p className="text-white text-sm font-medium mt-0.5">
                    {nextDayExercises.filter(e => e.exercise_type === 'corrida').length > 0
                      ? `Corrida Ã¢ÂÂ ${RUNNING_TYPE_LABELS[nextDayExercises.find(e => e.exercise_type === 'corrida')?.running_type]}`
                      : `MusculaÃÂ§ÃÂ£o Ã¢ÂÂ ${nextDayExercises.length} exercÃÂ­cios`}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="card border-primary/20 bg-primary/5">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-muted text-xs">Treino de Hoje Ã¢ÂÂ Dia {currentDay}</p>
                  <p className="text-white font-medium text-sm mt-0.5">
                    {todayExercises.filter(e => e.exercise_type === 'corrida').length > 0
                      ? `Corrida Ã¢ÂÂ ${RUNNING_TYPE_LABELS[todayExercises.find(e => e.exercise_type === 'corrida')?.running_type]}`
                      : `MusculaÃÂ§ÃÂ£o Ã¢ÂÂ ${todayExercises.length} exercÃÂ­cios`}
                  </p>
                </div>
                <button
        ...
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <h2 className="font-semibold text-white text-sm">EvoluÃÂ§ÃÂ£o Corrida</h2>
          </div>
            ...
            <>
              ...
              <div className="flex gap-2 mt-4 mb-3">
                {['stats', 'insights', 'history'].map(s => (
                  ...
                  >
                    {s === 'stats' ? 'Fase' : s === 'insights' ? 'Insights' : 'HistÃÂ³rico'}
                  </button>
                ))}
              </div>
              ...
              {runSection === 'stats' && (
                <div className="space-y-3">
                  {[
                    { label: 'DistÃÂ¢ncia da Fase', val: `${runLogs.reduce((s,l) => s + Number(l.distance_km||0), 0).toFixed(1)} km`, sub: `${runLogs.filter(l=>l.distance_km>0).length} corrida(s)` },
                    { label: 'Maior DistÃÂ¢ncia', val: `${Math.max(...runLogs.filter(l=>l.distance_km>0).map(l=>Number(l.distance_km)), 0)} km`, sub: 'na fase atual' },
                  ].map(s => (
